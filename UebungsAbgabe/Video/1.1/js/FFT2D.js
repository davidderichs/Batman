...
(function() {
  ...
  var core = {
    ...
    fftspectrum : function(spectrum,Smax,re, im, islog) {
      var val = 0,
          i = 0,
          p = 0,
          imax = 0.0,
      ...
      for(var i=0; i<n2; i++) {
        if(islog){
		 if (re[i] != 0 ||  im[i] != 0)  spectrum[i] = log(Math.sqrt(re[i]*re[i] + im[i]*im[i]));
		 else spectrum[i] = 0;
        } else {
 ...
 */     
  ...
  // aliases (public APIs)
  var apis = ['init', 'fft1d', 'ifft1d', 'fft2d', 'ifft2d', 'fftspectrum'];
...
(function() {
    ...
    BPF : function(re, im, radius, bandwidth) {
      ...
      for(var y=-n2; y<n2; y++) {
        ...
        for(var x=-n2; x<n2; x++) {
          r = sqrt(x*x + y*y);
          p = x + i;
          if(r < radius || r > ( bandwidth)) {
            re[p] = im[p] = 0;
          }
        }
      }
    },
	    // applies Band-Cut Filter
    BCF : function(re, im, radius, bandwidth) {
      var i = 0,
          p = 0,
          r = 0.0,
          n2 = _n >> 1,
      ...
      for(var y=-n2; y<n2; y++) {
	    ...
	    Quant : function(re, im, radius,LQuant,HQuant) {
      var i = 0,
          p = 0,
          r = 0.0,
          n2 = _n >> 1,
      ...
      for(var y=-n2; y<n2; y++) {
        ...
        for(var x=-n2; x<n2; x++) {
          ...
          p = x + i;
          if(r > radius) {
            re[p] = Math.round(re[p]/HQuant);
			im[p] = Math.round(im[p]/HQuant);
          } else {
		    re[p] = Math.round(re[p]/LQuant);
			im[p] = Math.round(im[p]/LQuant);
		  }
        }
      }
    },
  ...
  // aliases (public APIs)
  var apis = ['init', 'swap', 'HPF', 'LPF', 'BPF', 'BCF', 'Quant','windowing'];
  for(var i=0; i<apis.length; i++) {
